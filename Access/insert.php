...
<?php
	...
	$system = $username = $account = $password = $enteredby = $notes = $lastchangedate = "";
	$okay = true;
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST")
	{
		if (empty(trim($_POST["inputRequestor"]))) {
			...
			$enteredby = test_input($_POST["inputRequestor"]);
			// check if name only contains letters and whitespace
			if (!preg_match("/^[a-zA-Z-' ]*$/", $enteredby)) {
		...
		if (empty(trim($_POST["inputSystem"]))) {
			...
			$system = test_input($_POST["inputSystem"]);
		}

		if (empty(trim($_POST["inputUsername"]))) {
			$changesErr = "Enter the Username";
			$okay = false;
		} else {
...
//			$changes = mysql_real_escape_string(test_input($_POST["inputChanges"]));
		...
		if (empty(trim($_POST["inputPassword"]))) {
			$affectedErr = "Enter the Password";
			$okay = false;
		} else {
...
*/
		$lastchangedate = $_POST["inputDate"];
		...
		if ($okay)
		{
			...
			$sql = "INSERT INTO ITAccess (System, Entrypoint, User, Account, Password, EnteredBy, Notes, LastChangeDate) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
			if ($stmt = $link->prepare($sql))
			{
				$stmt->bind_param("sssd", $system, " ", " ", $account, $password, $username, " ", $lastchangedate);
...
<div class="row m-4">
	<div class="col-md-12">
		<h1 class="text-center">System Access</h1>
	</div>
</div>
	...
	<div class="card-body">
		<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
			...
			<div class="form-group">
				...
				<textarea class="form-control" id="inputUsername" name="inputUsername" rows="1" required></textarea>
				<span class="error"><?php echo $changesErr; ?></span>
			</div>

			<div class="form-group">
				<label for="inputPasword">Password: </label>
				<textarea class="form-control" id="inputPassword" name="inputPassword" rows="1" required></textarea>
